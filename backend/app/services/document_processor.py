...
async def preview_document(file_path: str, chunk_size: int = 1000, chunk_overlap: int = 200) -> PreviewResult:
    ...
    # 1. Create the temp file entry
    temp_file = tempfile.NamedTemporaryFile(delete=False, suffix=ext)
    temp_path = temp_file.name
    
    try:
        # 2. CLOSE the handle immediately. 
        # This releases the Windows lock so MinIO can write to the path.
        temp_file.close() 
        
        # 3. Download from MinIO to the now-available path
        ...
        # 4. Select appropriate loader using the path
        if ext == ".pdf":
        ...
        # 5. Manual cleanup of the file on disk
...
async def process_document_background(
    ...
    try:
        ...
        try:
            ...
            embeddings = EmbeddingsFactory.create()
            logger.info(f"Task {task_id}: embeddings initialized")
            ...
            logger.info(f"Task {task_id}: vector store initialized")
