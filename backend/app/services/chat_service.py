...
from app.services.llm.llm_factory import LLMFactory, TitleLLMFactory
...
async def generate_and_save_chat_title_bg(chat_id: int, first_message_content: str):
    """
    Runs in the background to generate a title and update the DB independently.
    """
    try:
        llm = TitleLLMFactory.create()
        
        prompt = PromptTemplate.from_template(
            "TASK: Generate a 3-word title for this chat based on the user's first message.\n"
            "RESTRICTION: Return ONLY the title. No quotes. No preamble.\n"
            "MESSAGE: {query}"
        )
        
        chain = prompt | llm | StrOutputParser()         
        response = await chain.ainvoke({"query": first_message_content})        
        generated_title = response.strip(' "\'\n')
        ...
        try:
            ...
            if chat:
                ...
                db.commit()
                logger.info(f"Updated title for chat {chat_id} to: {generated_title}")
        except Exception as db_err:
            logger.error(f"Database error updating chat title: {db_err}")
            db.rollback()
        finally:
            db.close()
